...
class APIContractValidator:
            ...
            with warnings.catch_warnings(record=True) as warning_list:
                ...
                if deprecation_warnings:
                    self.log_test(
                        test_name,
                        "PASS",
                        f"Correctly issued {len(deprecation_warnings)} deprecation warning(s)",
                    )
                else:
                    self.log_test(
                        test_name,
                        "WARN",
                        "No deprecation warnings issued - check deprecated_params.json",
                    )
    ...
    def test_parameter_filtering(self):
        ...
        try:
            ...
            filtered_params = deprecation_manager.filter_deprecated_params(
                "ChatCompletionRequest", original_params
            )
            ...
            if "max_tokens" in original_params and "max_tokens" not in filtered_params:
                self.log_test(
                    test_name, "PASS", "Deprecated parameters correctly filtered out"
                )
            else:
    ...
    async def test_live_api_compatibility(self):
        ...
        try:
            ...
            if hasattr(response, "choices") and response.choices:
                ...
                self.log_test(
                    test_name, "PASS", f"API call successful: '{content.strip()}'"
                )
            else:
                # Debug response structure
                print(f"DEBUG: Response type: {type(response)}")
                print(
                    f"DEBUG: Response attributes: {list(response.__dict__.keys()) if hasattr(response, '__dict__') else 'No __dict__'}"
                )
                self.log_test(
                    test_name, "FAIL", f"Invalid response structure: {type(response)}"
                )
            ...
            with warnings.catch_warnings(record=True) as warning_list:
                ...
                if capability_warnings:
                    self.log_test(
                        test_name,
                        "PASS",
                        f"Model validation working: {len(capability_warnings)} warnings",
                    )
                else:
                    self.log_test(
                        test_name, "PASS", "All parameters supported by model"
                    )
    ...
    def test_new_parameter_support(self):
        ...
        try:
            ...
            if "unexpected keyword argument" in str(e):
                self.log_test(
                    test_name, "FAIL", f"New parameter not supported in client: {e}"
                )
            else:
            ...
            self.log_test(
                test_name,
                "PASS",
                f"Client accepts parameter (API error: {type(e).__name__})",
            )
    ...
    def validate_spec_compliance(self):
        ...
        try:
            ...
            if chat_schema:
                ...
                self.log_test(
                    test_name, "FAIL", "ChatCompletionRequest schema not found in spec"
                )
    ...
    async def run_all_tests(self):
        ...
        for status, count in status_counts.items():
            emoji = {"PASS": "✅", "FAIL": "❌", "SKIP": "⏭️", "WARN": "⚠️"}.get(
                status, "❓"
            )
