...
class ModelAuditor:
    ...
    def extract_response_structure(
    ...
    def compare_model_vs_response(self, endpoint: str) -> Dict[str, Any]:
        ...
        if not config.get("expected_model"):
            return {
                "status": "no_model",
                "error": "No model defined for this endpoint",
    ...
    def run_audit(self) -> Dict[str, Any]:
        ...
        for endpoint in self.test_cases:
            ...
            if result.get("model_creation_success"):
                ...
                print(
                    f"❌ {endpoint}: Model mismatch - {result.get('model_error', 'Unknown error')}"
                )
                if result.get("missing_in_model"):
                    print(
                        f"   Missing in model: {', '.join(result['missing_in_model'][:5])}"
                    )
                if result.get("missing_in_response"):
                    print(
                        f"   Missing in response: {', '.join(result['missing_in_response'][:5])}"
                    )
