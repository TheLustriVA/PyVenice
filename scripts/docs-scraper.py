...
class VeniceDocsScraper:
    ...
    def extract_api_info(self, html: str, endpoint_path: str) -> Dict[str, Any]:
        ...
        info = {
            ...
            "description": None,
            "parameters": {},
            "examples": {"request": None, "response": None},
            "validation_rules": [],
            "models": [],
        }
        ...
        for match in model_mentions:
            ...
            if any(
                ...
                for keyword in ["venice", "flux", "gpt", "claude", "llama", "gemini"]
            ):
    ...
    async def scrape_endpoint_docs(self, endpoint_paths: List[str]) -> Dict[str, Any]:
        ...
        for endpoint_path in endpoint_paths:
            ...
            docs_url = (
                f"{self.base_url}/api-reference/endpoint/{clean_path.replace('/', '/')}"
            )
    ...
    def merge_with_swagger(
        self, swagger_spec: Dict[str, Any], scraped_docs: Dict[str, Any]
    ) -> Dict[str, Any]:
            ...
            if swagger_path:
                ...
                for method, method_info in path_info.items():
                    if method.lower() in ["get", "post", "put", "delete", "patch"]:
                        ...
                        if docs_info["description"] and len(
                            docs_info["description"]
                        ) > len(method_info.get("description", "")):
                            method_info["description"] = docs_info["description"]

                        # Add examples
                        if docs_info["examples"]["request"]:
                            if "requestBody" in method_info:
                                method_info["requestBody"]["content"] = method_info[
                                    "requestBody"
                                ].get("content", {})
                                for content_type in method_info["requestBody"][
                                    "content"
                                ]:
                                    method_info["requestBody"]["content"][content_type][
                                        "example"
                                    ] = docs_info["examples"]["request"]

                        if docs_info["examples"]["response"]:
                            method_info["responses"] = method_info.get("responses", {})
                            for status_code, response_info in method_info[
                                "responses"
                            ].items():
    ...
    def save_enhanced_spec(
        self, enhanced_spec: Dict[str, Any], filename: str = "swagger_enhanced.yaml"
    ):
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "total_parameters": sum(
                len(info["parameters"]) for info in scraped_docs.values()
            ),
            "total_examples": sum(
                1
                for info in scraped_docs.values()
                if info["examples"]["request"] or info["examples"]["response"]
            ),
            "total_validation_rules": sum(
                len(info["validation_rules"]) for info in scraped_docs.values()
            ),
...
async def main():
    ...
    parser.add_argument(
        "--endpoint", action="append", help="Specific endpoint to scrape"
    )
    parser.add_argument(
        "--output", default="swagger_enhanced.yaml", help="Output filename"
    )
