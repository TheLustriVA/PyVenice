...
try:
    ...
    safety_spec = importlib.util.spec_from_file_location(
        "safety_validator", Path(__file__).parent / "safety-validator.py"
    )
    ...
    ci_spec = importlib.util.spec_from_file_location(
        "ci_feedback", Path(__file__).parent / "ci-feedback.py"
    )
...
class SafeAutoDeployer:
    ...
    async def detect_api_changes(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                has_changes = (
                    any(changes.get("endpoints", {}).values())
                    or any(changes.get("schemas", {}).values())
                    or any(changes.get("parameters", {}).values())
                )
    ...
    async def run_safety_validation(
        ...
        try:
            validation_passed, backup_dir = (
                await self.safety_validator.comprehensive_validation(changes)
            )
...
Co-Authored-By: Claude <noreply@anthropic.com>"""
    ...
    async def merge_to_main(self, branch_name: str) -> bool:
        ...
        try:
            # Switch to main
            subprocess.run(
                ["git", "checkout", "main"], cwd=self.project_root, check=True
            )
            subprocess.run(
                ["git", "pull", "origin", "main"], cwd=self.project_root, check=True
            )
    ...
    async def cleanup_deployment(
        self, branch_name: str, backup_dir: Optional[Path] = None
    ):
    ...
    async def rollback_deployment(
        self, backup_dir: Optional[Path], branch_name: str = ""
    ):
    ...
    async def run_safe_deployment(self) -> bool:
        ...
        try:
            ...
            self.log_step(
                "Deployment", "PASS", "Safe deployment completed successfully"
            )
            return True
...
async def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Skip safety checks (NOT RECOMMENDED)"
    )
