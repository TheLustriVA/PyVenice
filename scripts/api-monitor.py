...
class APIMonitor:
    ...
    def compare_parameters(
        ...
        param_changes = {"added": [], "removed": [], "modified": []}
        ...
        for schema_name in all_schemas:
            ...
            for param_name in old_param_names & new_param_names:
                if (
                    old_schema_params[param_name]["hash"]
                    != new_schema_params[param_name]["hash"]
                ):
    ...
    def compare_specs(
        ...
        changes["schemas"]["added"] = sorted(
            set(new_schemas.keys()) - set(old_schemas.keys())
        )
        changes["schemas"]["removed"] = sorted(
            set(old_schemas.keys()) - set(new_schemas.keys())
        )
        ...
        if total_changes == 0:
            changes["summary"] = (
                f"No changes detected (version: {changes['version_change']['old']} → {changes['version_change']['new']})"
            )
        else:
            parts = []
            ...
            if changes["endpoints"]["removed"]:
                parts.append(
                    f"{len(changes['endpoints']['removed'])} removed endpoints"
                )
            ...
            if changes["parameters"]["removed"]:
                parts.append(
                    f"{len(changes['parameters']['removed'])} removed parameters"
                )
            if changes["parameters"]["modified"]:
                parts.append(
                    f"{len(changes['parameters']['modified'])} modified parameters"
                )
    ...
    def print_changes_report(self, changes: Dict[str, Any]):
        ...
        if changes["version_change"]["old"] != changes["version_change"]["new"]:
            print(
                f"📝 Version: {changes['version_change']['old']} → {changes['version_change']['new']}"
            )
            print()
    ...
    async def run(self, save_changes: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if save_changes:
                ...
                if any(changes["endpoints"].values()) or any(
                    changes["schemas"].values()
                ):
...
async def run_changelog_monitoring():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "changelog_monitor", Path(__file__).parent / "changelog-monitor.py"
        )
        ...
        if changelog_report.get("status") == "success":
            ...
            if changelog_report["api_relevant_new"] > 0:
                print(
                    f"   ⚠️  API-relevant changes: {changelog_report['api_relevant_new']}"
                )
            ...
            print(
                f"   ❌ Changelog monitoring failed: {changelog_report.get('error', 'Unknown error')}"
            )
    except Exception as e:
...
async def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Don't save changes, just report them"
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    if "endpoints" in changes and "schemas" in changes:
        total_changes = (
            len(changes["endpoints"].get("added", []))
