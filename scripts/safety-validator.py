...
class SafetyValidator:
    ...
    def validate_syntax(self, file_path: Path) -> bool:
        ...
        try:
            ...
            self.log_validation(
                f"Syntax Check: {file_path.name}", "FAIL", f"Line {e.lineno}: {e.msg}"
            )
            return False
...
"""
    ...
    def validate_function_signatures(
        self, file_path: Path, expected_functions: List[str] = None
    ) -> bool:
        ...
        try:
            ...
            if expected_functions:
                ...
                if missing:
                    self.log_validation(
                        f"Function Check: {file_path.name}",
                        "FAIL",
            ...
            if result.returncode == 0:
                ...
                try:
                    ...
                    if error_count > 0:
                        ...
                        self.log_validation(
                            "Code Quality", "WARN", f"{warning_count} warnings"
                        )
                        return True
    ...
    def create_safety_backup(self) -> Path:
        ...
        try:
            ...
            self.log_validation(
                "Safety Backup", "PASS", f"Backup created: {backup_dir}"
            )
            return backup_dir
    ...
    def restore_from_backup(self, backup_dir: Path) -> bool:
        ...
        try:
            if not backup_dir.exists():
                self.log_validation(
                    "Backup Restore", "FAIL", "Backup directory not found"
                )
                return False
            ...
            self.log_validation(
                "Backup Restore", "PASS", "Successfully restored from backup"
            )
            return True
    ...
    async def comprehensive_validation(
        ...
        for status, count in status_counts.items():
            emoji = {"PASS": "✅", "FAIL": "❌", "WARN": "⚠️", "SKIP": "⏭️"}.get(
                status, "❓"
            )
...
async def main():
    ...
    if success:
        ...
        if backup_dir:
            print(
                f"   Restore command: python scripts/safety-validator.py --restore {backup_dir}"
            )
