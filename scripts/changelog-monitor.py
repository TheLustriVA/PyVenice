...
class VeniceChangelogMonitor:
    def __init__(self):
        ...
        if not self.username or not self.password:
            raise ValueError(
                "VENICE_USERNAME and VENICE_PASS environment variables required"
            )
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                if (
                    ...
                    or "Enable JavaScript" in page_content
                ):
                ...
                if (
                    "Just a moment..." not in final_content
                    and "Enable JavaScript" not in final_content
                ):
                ...
                for selector in login_selectors:
                    try:
                        login_element = await page.wait_for_selector(
                            ...
                            if email_input and password_input:
                                ...
                                try:
                                    ...
                                    if response.status == 200:
                                        print(
                                            "✅ Successfully authenticated and accessed changelog"
                                        )
        ...
        try:
            ...
            if not changelog_entries:
                ...
                date_pattern = re.compile(
                    r"\b\d{4}[-/]\d{1,2}[-/]\d{1,2}\b|\b\w+ \d{1,2},? \d{4}\b"
                )
            ...
            for i, entry_elem in enumerate(changelog_entries[:20]):  # Limit entries
                try:
                    ...
                    title = (
                        title_elem.get_text(strip=True) if title_elem else content[:100]
                    )
    ...
    def detect_new_entries(
        self, current_entries: List[Dict[str, Any]], cached_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
