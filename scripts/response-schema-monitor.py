...
class ResponseSchemaMonitor:
    ...
    def monitor_schemas(self) -> Dict[str, Any]:
            ...
            if endpoint in cached_data:
                ...
                if cached_schema and current_schema:
                    ...
                    if changes["total_changes"] > 0:
                        ...
                        print(
                            f"   Added: {len(changes['added_fields'])}, Removed: {len(changes['removed_fields'])}, Type changes: {len(changes['type_changes'])}"
                        )
                    else:
        ...
        report = {
            ...
            "current_schemas": {
                ep: data.get("schema_hash", "") for ep, data in current_schemas.items()
            },
        }
